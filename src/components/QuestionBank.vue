                    ...
                    <el-container>
                        <el-header style="position: fixed;">
                            <div style="display: flex; background-color: white;">
                                ...
                                <el-divider direction="vertical" style="height: 40px;" />
                                <el-input v-model="searchText" placeholder="在题库中搜索" :prefix-icon="Search"
...
import { questionsStore } from '../store/question';
...
export default {
    ...
    data() {
        return {
            knowledgeStore,
            knowledgeList: knowledgeStore.data.subject,
            searchText: "",
            clickAType: "default",
            clickBType: "default",
            clickCType: "default",
            clickDType: "default",
            questionsStore,
            noData: questionsStore.data.noData,
            questionList: questionsStore.data.questions,
            question: {
                index: 0,
                question_id: "",
                knowledge_id: "",
    ...
    created() {
        var this_vue = this;
        requestService({
            url: "/list/getKnowledgeList",
            method: "get",
        }).then(function (response) {
            let res = response.data;
            knowledgeStore.data.subject = res;
            this_vue.knowledgeList = knowledgeStore.data.subject;
            questionsStore.data.questions = [];
            questionsStore.data.noData = true;
            this_vue.noData = questionsStore.data.noData;
            this_vue.questionList = questionsStore.data.questions;
        })
    },
    methods: {
        handleA() {
        ...
        async loadQuestion(index) {
            ...
            const response = await requestService({
                url: "/list/getQuestionList?knowledgeId=" + index,
                method: "get",
            });
            let res = response.data;
            if (res.length > 0) {
                ...
                questionsStore.data.questions = res;
                questionsStore.data.noData = false;
            }
            else {
                questionsStore.data.questions = [];
                questionsStore.data.noData = true;
        ...
        async searchToQuestion(question) {
            ...
            var this_vue = this;
            const response = await requestService({
                url: "/list/getQuestionList?knowledgeId=" + kId,
                method: "get",
            });
            let res = response.data;
            let qIndex = 0;
