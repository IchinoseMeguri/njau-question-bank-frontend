                    ...
                    <el-container>
                        <el-header style="position: fixed;">
                            <div style="display: flex; background-color: white;">
                                ...
                                <el-divider direction="vertical" style="height: 40px;" />
                                <el-input v-model="searchText" placeholder="在题目收藏中搜索" :prefix-icon="Search"
...
export default {
    ...
    data() {
        return {
            collectStore,
            knowledgeList: collectStore.data.subject,
            searchText: "",
            clickAType: "default",
            clickBType: "default",
            clickCType: "default",
    ...
    created() {
        var this_vue = this;
        requestService({
            url: "/collect/getCollectList?userId=" + loginInfoStore.data.user_id,
            method: "get",
        }).then(function (response) {
            let res = response.data;
            collectStore.data.subject = res;
            this_vue.knowledgeList = collectStore.data.subject;
        })
    },
    methods: {
        handleA() {
        ...
        searchQuestion() {
            ...
            let this_vue = this;
            return requestService({
                url: "/collect/search?userId=" + loginInfoStore.data.user_id,
                method: "get",
